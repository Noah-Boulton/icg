...
//    int numLights = 4;
//    Vec3 lightPos[4] = {Vec3(2.0f, 9.0f, 2.0f), Vec3(-2.0f, 9.0f, 2.0f), Vec3(0.0f, 9.0f, 2.0f), Vec3(-1.5f, 9.0f, -2.0f)};
    int numLights = 3;
 ...
 * @param shapes - shape array to check intersection
 * @param numShapes - number of shapes in the scene
...
Colour trace(Vec3 &ray, shape * shapes[], int numShapes, Vec3 &origin, Vec3 lightPos[], float lightInt, int depth, float ambient, int numLights){
    ...
    if(shapes[index]->get_glass()){ // Glass refraction
        ...
        Vec3 hitPosSubC = hitPos - shapes[index]->get_center();
        float t = shapes[index]->intersect(hitPos, hitPosSubC);
        if(t = 0){
            return black();
        }
        Vec3 hitPos2 = hitPos + refract*t;
        norm = shapes[index]->get_normal(hitPos2);
        ...
        shade += trace(refract2, shapes, numShapes, hitPos, lightPos, lightInt, depth+1, ambient, numLights);
    }
